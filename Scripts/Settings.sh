...
1. 修改国家代码 - 更精确的匹配
找到设置country的行并修改
sed -i "s/country='${country || ''}'/country='US'/g" "$WIFI_UC"
2. 修改功率设置 - 在正确的位置插入
先检查是否已存在txpower设置
...
3. 修改通道设置 - 更精确的匹配
echo "=== 当前channel设置逻辑 ==="
grep -A 10 -B 5 "let channel" "$WIFI_UC"
修改channel生成逻辑 - 更精确的替换
sed -i 's/let channel = rband.default_channel ?? "auto";/let channel = (band_name == "5G") ? "44" : (band_name == "2G") ? "9" : rband.default_channel ?? "auto";/g' "$WIFI_UC"
4. 修改通道宽度 - 需要更精确的处理
首先查看当前的htmode设置逻辑
echo "=== 当前htmode设置逻辑 ==="
...
echo "=== 通道设置 ==="
grep -A 5 -B 5 "channel.=" "$WIFI_UC" || echo "未找到channel设置"
echo "=== 带宽设置逻辑 ==="
